...
class ProjectContainer {
  ...
  initTabManager() {
    ...
    this.tabManager.registerAll({
      ...
      preview: () => import('./views/preview-view.js'),
  ...
  setupEventListeners() {
    ...
    navNotes?.addEventListener('click', async () => {
      await this.switchTab('research', true);
    });

    navBoard?.addEventListener('click', async () => {
      await this.switchTab('kanban', true);
    });

    navPreview?.addEventListener('click', async () => {
      await this.switchTab('preview', true);
    });

    navChat?.addEventListener('click', async () => {
      await this.switchTab('chat', true);
    });
  ...
  updateActiveTab(tabName) {
    ...
    // Sync active state to bottom nav (only for tabs)
    const isTab = tabName === 'research' || tabName === 'kanban' || tabName === 'preview' || tabName === 'chat' || tabName === 'settings';
    if (isTab) {
      const navNotes = document.getElementById('navNotes');
      const navBoard = document.getElementById('navBoard');
      const navPreview = document.getElementById('navPreview');
      ...
      navNotes?.classList.toggle('active', tabName === 'research');
      navBoard?.classList.toggle('active', tabName === 'kanban');
      navPreview?.classList.toggle('active', tabName === 'preview');
  ...
  /**
   * Open (or create) the live preview for this project
   */
  async openPreview() {
    try {
      // 1. Get the project's board to find built tasks
      const boardData = await api.get(`/projects/${this.projectId}/board`, { skipCache: true });
      const board = boardData.board;

      // 2. Look for built tasks in "building" or "done" columns (prefer most recent)
      const builtTasks = [
        ...(board.building || []),
        ...(board.done || [])
      ];
      ...
      if (builtTasks.length === 0) {
        toast.error('No built tasks found. Build a task from the Kanban board first.');
        return;
      }

      // 3. Get the most recent task with a generation
      let targetTask = null;
      ...
      if (!targetTask || !targetTask.generations || targetTask.generations.length === 0) {
        toast.error('No built generation found. Please build a task first.');
        return;
      }

      // 4. Get the sandbox URL from the most recent generation
      const generation = targetTask.generations[0];
      if (!generation.sandboxId) {
        toast.error('Sandbox not available for this generation.');
        return;
      }

      // Construct E2B sandbox URL
      const sandboxUrl = `https://${generation.sandboxId}.e2b.dev:8000`;
      console.log('Opening preview for:', sandboxUrl);

      // 5. Navigate directly (most reliable way to open in browser)
